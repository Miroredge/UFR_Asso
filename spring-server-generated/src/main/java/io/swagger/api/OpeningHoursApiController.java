...
@RestController
public class OpeningHoursApiController implements OpeningHoursApi {

    private static final Logger log = LoggerFactory.getLogger(OpeningHoursApiController.class);
    ...
    public OpeningHoursApiController(ObjectMapper objectMapper, HttpServletRequest request) {
        this.objectMapper = objectMapper;
        this.request = request;
    }

    public ResponseEntity<Errors> deleteOpeningHour(@Parameter(in = ParameterIn.HEADER, description = "Network location of the URI" ,required=true,schema=@Schema()) @RequestHeader(value="Host", required=true) String host,@Size(min=1,max=8) @Parameter(in = ParameterIn.PATH, description = "Technical Unique ID of the 'OpeningHour time' slot (Base36 encoding) (https://en.wikipedia.org/wiki/Base36)", required=true, schema=@Schema()) @PathVariable("openingHourId") String openingHourId) {
    ...
    public ResponseEntity<OpeningHourFound> getOpeningHourByID(@Parameter(in = ParameterIn.HEADER, description = "Network location of the URI" ,required=true,schema=@Schema()) @RequestHeader(value="Host", required=true) String host,@Size(min=1,max=8) @Parameter(in = ParameterIn.PATH, description = "Technical Unique ID of the 'OpeningHour' time slot (Base36 encoding) (https://en.wikipedia.org/wiki/Base36)", required=true, schema=@Schema()) @PathVariable("openingHourId") String openingHourId) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<OpeningHourFound>(objectMapper.readValue("\"\"", OpeningHourFound.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<OpeningHourFound>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<OpeningHourFound>(HttpStatus.NOT_IMPLEMENTED);
    }

    public ResponseEntity<OpeningHourFound> overwriteOpeningHourByID(@Parameter(in = ParameterIn.HEADER, description = "Network location of the URI" ,required=true,schema=@Schema()) @RequestHeader(value="Host", required=true) String host,@Size(min=1,max=8) @Parameter(in = ParameterIn.PATH, description = "Technical Unique ID of the 'OpeningHour' time slot (Base36 encoding) (https://en.wikipedia.org/wiki/Base36)", required=true, schema=@Schema()) @PathVariable("openingHourId") String openingHourId,@Parameter(in = ParameterIn.DEFAULT, description = "'OpeningHour' detailled content", required=true, schema=@Schema()) @Valid @RequestBody OpeningHour body) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<OpeningHourFound>(objectMapper.readValue("\"\"", OpeningHourFound.class), HttpStatus.NOT_IMPLEMENTED);
